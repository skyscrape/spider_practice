...
class TokenManager:
    ...
    async def gen_real_msToken(self) -> str:
        ...
        async with AsyncHTTPClient() as client:
            post_data = {
                ...
                "version": 1,
                "dataType": 8,
                "strData": config.DOUYIN_MS_TOKEN_REQ_STR_DATA,
            ...
            response = await client.post(
                config.DOUYIN_MS_TOKEN_REQ_URL, json=post_data, headers=headers
            )
    ...
    async def gen_webid(self) -> str:
        ...
        async with AsyncHTTPClient() as client:
            ...
            try:
                response = await client.post(
                    config.DOUYIN_WEBID_REQ_URL, json=post_data, headers=headers
                )
...
async def test_token_manager():
    user_agent = config.DOUYIN_FIXED_USER_AGENT
    token_manager = TokenManager(user_agent)
    ms_token = await token_manager.get_msToken()
    webid = await token_manager.gen_webid()
    verify_fp = VerifyFpManager.gen_verify_fp()
